...
function Welcome() {
  ...
  const { setUser } = useUser(); // Get setUser from global context to store the user
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      if (data.session?.user) {
        setUser(data.session.user); // Set user in context if session exists
      }
    };

    checkSession(); // Check if there's a valid session
  }, [setUser]);
      ...
      <div className="w-1/2 flex flex-col justify-center relative">
        ...
        <div className="w-4/5 mx-auto p-8">
          ...
          <div className="flex space-x-4 mt-4">
            <BackgroundButton text="Google" image={GoogleSVG} flip />
            <BackgroundButton text="Apple" image={AppleSVG} flip />
          </div>
          ...
          <div className="mt-8">
            ...
            <div className="mt-6">
              <button onClick={handleLogin} className="w-full bg-blue-500 text-white py-2 rounded-full">
                Start Learning
              </button>
            </div>

            {/* Display session information if logged in */}
            {session && (
