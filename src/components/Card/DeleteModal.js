...
function DeleteModal({ isOpen, onClose, onDelete, text }) {
    ...
    const handleDelete = (event) => {
        ...
        handleClose(event); // Close the modal after deletion
    };
    ...
    return ReactDOM.createPortal(
        <div
            ...
            onClick={handleClose} // Close modal if the background is clicked
        >
            {/* Black Background */}
            <div
                className="absolute inset-0 bg-black bg-opacity-50 transition-opacity duration-300"
                onClick={handleClose} // Ensure clicking inside the modal also doesn't propagate
            ></div>
            ...
            >
                ...
                <div className="flex justify-end space-x-4">
                    <BackgroundButton text="Cancel" bgColor="gray" onClick={(e) => handleClose(e)} />
                    <BackgroundButton text="Delete" bgColor="red" onClick={(e) => handleDelete(e)} />
                </div>
            </div>
        </div>,
